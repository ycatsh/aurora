...
class Shop(commands.Cog):
    ...
    async def shop_info(self, ctx, message):
        if message.lower() == "dp":
            ...
            error_msg.add_field(name="Error", value="please enter a valid item's ID to get more information on it\nFormat: ``.shop info <id>``\nSupport Server: [Join!](https://discord.gg/G9vTrsV4aG)")
    ...
    async def buy(self, ctx):
        error_msg = discord.Embed(title=" ", description=" ", color=0xffb12b)
        error_msg.add_field(name="Error", value="please enter a valid amount and a valid ID to buy an item\nFormat: ``.buy <item id> <amount>``\nSupport Server: [Join!](https://discord.gg/G9vTrsV4aG)")
        await ctx.reply(embed=error_msg, mention_author=False)
    ...
    async def buy_dp(self, ctx, *, message):
        try:
            message = int(message)
            self.cursor.execute("select durapot from users where client_id = %s", [ctx.author.id])
            ...
            if message < 1:
                ...
                if money < 0:
                    ...
                    self.cursor.execute("UPDATE users set user_coins = %s, durapot = %s where client_id = %s", [money, buy, ctx.author.id])
            ...
            error_msg = discord.Embed(title=" ", description=" ", color=0xffb12b)
            error_msg.add_field(name="Error", value="please enter a valid amount of the item you're trying to buy \nSupport Server: [Join!](https://discord.gg/G9vTrsV4aG)")
            await ctx.reply(embed=error_msg, mention_author=False)
    ...
    async def buy_hp(self, ctx, *, message):
        try:
            message = int(message)
            self.cursor.execute("select healpot from users where client_id = %s", [ctx.author.id])
            result = self.cursor.fetchall()
            self.cursor.execute("select user_coins from users where client_id = %s", [ctx.author.id])
            result2 = self.cursor.fetchall()
            ...
            if message < 1:
                ...
                if money < 0:
                    ...
                    self.cursor.execute("UPDATE users set user_coins = %s, healpot = %s where client_id = %s", [money, buy, ctx.author.id])
    ...
    async def buy_vp(self, ctx, *, message):
        try:
            message = int(message)
            self.cursor.execute("select vigopot from users where client_id = %s", [ctx.author.id])
            result = self.cursor.fetchall()
            self.cursor.execute("select user_coins from users where client_id = %s", [ctx.author.id])
            result2 = self.cursor.fetchall()
            ...
            if message < 1:
                ...
                if money < 0:
                    ...
                    self.cursor.execute("UPDATE users set user_coins = %s, vigopot = %s where client_id = %s", [money, buy, ctx.author.id])
    ...
    async def buy_qg(self, ctx, *, message):
        try:
            message = int(message)
            self.cursor.execute("select quagun from users where client_id = %s", [ctx.author.id])
            result = self.cursor.fetchall()
            self.cursor.execute("select user_coins from users where client_id = %s", [ctx.author.id])
            result2 = self.cursor.fetchall()
            ...
            if message < 1:
                ...
                if money < 0:
                    ...
                    self.cursor.execute("UPDATE users set user_coins = %s, quagun = %s where client_id = %s", [money, buy, ctx.author.id])
    ...
    async def buy_eb(self, ctx, *, message):
        try:
            message = int(message)
            self.cursor.execute("select enebazo from users where client_id = %s", [ctx.author.id])
            result = self.cursor.fetchall()
            self.cursor.execute("select user_coins from users where client_id = %s", [ctx.author.id])
            result2 = self.cursor.fetchall()
            ...
            if message < 1:
                ...
                if money < 0:
                    ...
                    self.cursor.execute("UPDATE users set user_coins = %s, enebazo = %s where client_id = %s", [money, buy, ctx.author.id])
    ...
    async def buy_orb(self, ctx, *, message):
        try:
            message = int(message)
            self.cursor.execute("select orb from users where client_id = %s", [ctx.author.id])
            result = self.cursor.fetchall()
            self.cursor.execute("select user_coins from users where client_id = %s", [ctx.author.id])
            result2 = self.cursor.fetchall()
            ...
            if message < 1:
                ...
                if money < 0:
                    ...
                    self.cursor.execute("UPDATE users set user_coins = %s, orb = %s where client_id = %s", [money, buy, ctx.author.id])
    ...
    async def buy_molecule(self, ctx, *, message):
        try:
            message = int(message)
            self.cursor.execute("select molecule from users where client_id = %s", [ctx.author.id])
            result = self.cursor.fetchall()
            self.cursor.execute("select user_coins from users where client_id = %s", [ctx.author.id])
            result2 = self.cursor.fetchall()
            ...
            if message < 1:
                ...
                if money < 0:
                    ...
                    self.cursor.execute("UPDATE users set user_coins = %s, molecule = %s where client_id = %s", [money, buy, ctx.author.id])
    ...
    async def use(self, ctx):
        error_msg = discord.Embed(title=" ", description=" ", color=0xffb12b)
        error_msg.add_field(name="Error", value="please enter a valid ID to use an item\nFormat: ``.use <item id> <quantity>``\n``quantity`` doesnt apply for these items: ``orb``, ``box``\nSupport Server: [Join!](https://discord.gg/G9vTrsV4aG)")
        await ctx.reply(embed=error_msg, mention_author=False)
        ...
        try:
            ...
            if message < 0:
                ...
                if dpi < 0:
                    ...
                    self.cursor.execute("update users set durapot = %s, defense = %s where client_id = %s", [
        ...
        try:
            ...
            if message < 0:
                ...
                if hpi < 0:
                    ...
                    self.cursor.execute("update users set healpot = %s, health = %s where client_id = %s", [
        ...
        try:
            ...
            if message < 0:
                ...
                if vpi < 0:
                    ...
                    self.cursor.execute("update users set vigopot = %s, attack = %s where client_id = %s", [
    ...
    async def use_orb(self, ctx):
        ...
        result = self.cursor.fetchall()
        self.cursor.execute("select orb from users where client_id = %s",
                    [ctx.author.id])
        result2 = self.cursor.fetchall()
    ...
    async def use_box(self, ctx):
        self.cursor.execute("select box from users where client_id = %s",
                    [ctx.author.id])
        ...
        if len(res) == 0:
            ...
            if rng == 1:
                ...
                self.cursor.execute("update users set user_coins = %s, healpot = %s, orb = %s, box = %s where client_id = %s", [
                ...
                self.cursor.execute("update users set user_coins = %s, quagun = %s, orb = %s, box = %s where client_id = %s", [
                ...
                self.cursor.execute("update users set user_coins = %s, durapot = %s, box = %s where client_id = %s", [
